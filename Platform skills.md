...
Source: the nine reusable skills of the target platform, as used in the SSG BRD (OTC Allege Pilot) and the skills-marketplace vision. Two of the nine exist as registered utilities in the SSG BRD: Unstructured to Structured (TDG1001567) and Compare and Match (TDG1001568). CSG raises one NTI for the project as a whole, covering all nine skills; the SSG NTIs are precedents, not extended.
...
| Unstructured to Structured | Extracts fields from email bodies, webform comment fields, Excel, PDF and Word attachments, including password-protected files. Same capability as the SSG utility; covered by the project NTI | 14 units by email body; 7 by Excel attachment; 3.15 multi-format attachments; 2.10 CRUD determination is the hardest case |
...
| Validate and Approve | (a) Rules-based checks with no AI in the path (mandatory fields, permitted values, entity lists, duplicate check). (b) The single human decision point: the checker, shown the comparison table (requested versus proposed), approves or rejects on their own credentials. (c) External approvals routed to Legal / FO / POC where the as-is requires them. Code-as-maker / human-as-checker is the control pattern inside this skill, not a separate utility | Rule content exists for area 1 only; checker drawn on 18 of 27 units; the single presentation requirement is on the 2.1 slide (approver receives a data comparison table contrasting requested against actual) |
| Calculate and Process | (a) Stages the change into the target system's pending state, or outside it where none exists, under a named automation identity. (b) Commits at checker approval on the checker's credentials: create, update, close, delete alt code, upload. Idempotent, with rollback and audit. Never commits without the checker | The most frequently drawn step type in the deck |
...
| Compare and Match | (a) Match against a reference source: GLEIF portal, PB sheet, F1SA, Totoro, duplicate check against EVE. (b) Cross-system position and linkage check before a closure or deletion. Phase 1 automated read covers EVE and Totoro only; the check against GMI, Venom-Viper, Loanet, Global1, Euclid and Nuvo is performed by the checker as part of approval, with the result recorded; automated in Phase 2 with 3.13. Same capability as the SSG utility; covered by the project NTI | (a) 2.15, 3.16, 3.17, 3.15, 3.4; (b) 2.7 both regions, 3.8 all regions |
...
| Route and Distribute | Assignment of the staged change to a checker, escalation to Legal / FO / KYC, region routing | 2.13 Legal ticket; 3.8 FO approval per TA; 3.10 POC approvals; 2.7 and 3.8 regions |
...
| U6 maker-checker presentation | Validate and Approve (b), the checker decision point; the maker is the automation |
...
- Control model (decided 16 Sep): code as maker, human as checker. The automation writes only into pending or staged states under a named automation identity; the checker's approval on their own credentials commits the change. Each configuration has exactly one human decision point. Acceptance of this model by ORM and under the AI Governance Standard is a prerequisite.
- NTI: one NTI for the CSG EDM automation as a whole, covering all nine skills; the SSG NTIs are cited as precedents only.
- Sub-modes are named per skill and to-be steps tag at sub-mode level; a to-be may use one sub-mode of a skill and not the others.
