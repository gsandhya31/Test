...
- brd/01-capability-catalogue.md   Working file: the nine platform skills with evidence, sub-modes and open points. Corrected by every chapter. In Phase 3 it is rewritten as the BRD section "Solution capabilities" (brd/02-solution-capabilities.md: definitions, sub-modes, activities using each skill); evidence and open points move to log/.
...
- brd/annex-scope-register.md      Working file and live status table: every underlying activity as one row (one row per region where flows differ; 3.4 as two rows): status, scope decision, reason code, input pattern(s), as-is checker drawn, frequency, unblocking question, alias, slides. Rows ordered area 1, 2, 3, 4; sheet names verbatim. In Phase 3 the BRD annex (brd/annex-a-scope-register.md) is produced from it with business columns only (activity, name, scope, reason, input pattern, frequency, to-be yes/no); slides, aliases, checker column and questions stay internal.
...
## Chapter template (per Level 2 area), revised 18 Sep
1. Area overview: what the area does in three sentences, then ONE activity table (number, sheet name, scope decision, input pattern(s), frequency or "business to populate"). No other status or scope tables; the register holds the detail.
2. Activities: one sub-section per flow group, heading "2.n Activity <numbers>: <sheet names>". Regional variants are sub-sub-sections. Where a later chapter duplicates an earlier chapter's flow (3.2 = 2.2, 3.16 = 2.15, 3.6 reuses 2.4, 3.11 twin of 2.12, 3.7 = 2.6), the later chapter cross-references, it does not rewrite. Inside each sub-section, in this order:
   - As-is: input pattern and channel, frequency, checker today; then the steps as a numbered list of 6 to 10 lines.
   - To-be: the configuration shape (input, extraction, extraction gate, checks, staging, commit approval, outputs), each step tagged with skill and sub-mode; the checker's two decision points marked; rejection paths; every rule whose content is missing written as a named configurable check with "content: to be supplied by <owner>". Where the as-is is pending sign-off, say so in a [CAVEAT].
   - What changes for the team: three lines.
   - Open points for this activity: inline, each with activity number, slide number and owner.
3. Requirements: one FR table for the chapter, FR-<chapter>-<nn>, each testable and traced to a to-be step and skill.
4. Controls and governance, once per chapter: code as maker / human as checker, automation identity, checker credentials, comparison table, rejection path, audit trail; mapping to the AI Governance Standard, NTI and ORMF quoted verbatim with article or section numbers.
5. Data, systems and prerequisites: per target system the write mechanism and pending state; webform API for IP1; IP4 intake mechanism; ORM acceptance of the control model.
6. Open points roll-up: one table by owner, collected from the sub-sections.
Chapter 4 (Commission and Fees): a title, one paragraph, the activity list. No template sections.

## Writing rules (every chapter)
- Plain language, present tense, short sentences. Say what the process is, not where the evidence came from.
- Every fact once. If the register or catalogue already states it, reference it; do not repeat the table.
- [CAVEAT: ...] wraps anything about the evidence rather than the process: source conflicts, pending sign-off, slide or journal references, assumptions. Body text never narrates sources. Phase 3 produces the business version by stripping every [CAVEAT] tag and its contents; the internal version keeps them.
- No status-report phrasing ("sign off provided", "flow created", "as recorded"). Scope status appears once, in the section 1 table.
...
- decision points: the checker's extraction confirmation gate (on by default, wizard flag) and the checker's commit approval (mandatory); what the checker is shown at each; the rejection path from each
...
- Automation starts at request receipt, which is an email (IP1 to IP3) or an EVE work item (IP4): identify the activity, extract or fetch the request data, checker confirms extraction (gate), run checks, stage the change, checker approves and commits.
...
- Unstructured to Structured: extract fields from email bodies, form comment fields, Excel, PDF and Word attachments, including password-protected files (U2c). Same capability as the SSG utility (precedent NTI TDG1001567); covered by the single CSG NTI.
...
- Validate and Approve: (a) rules-based checks with no AI on the extracted and derived fields; (b1) extraction confirmation gate, the checker confirms extracted and derived fields, on by default and switchable off per configuration; (b2) commit approval, the checker sees the comparison table (2.1 slide requirement, quoted in the catalogue: the approver receives a data comparison table contrasting requested against actual) and approves or rejects on their own credentials; (c) external approvals routed to Legal / FO / POC where the as-is requires them. Code-as-maker / human-as-checker is the control pattern inside this skill, not a separate utility.
...
- Route and Distribute: assignment of the case to a checker for both decision points, escalation to Legal / FO / KYC, region routing.
...
- NTI: one NTI is raised for the CSG EDM automation as a whole, covering all nine skills and all Phase 1 configurations. No NTI per skill. The SSG NTIs (TDG1001567, TDG1001568) are cited as precedents only. The front section carries one NTI paragraph, not a table.
...
- Two human decision points per configuration, both performed by the checker (decided 17 Sep, supersedes "exactly one"): (1) extraction confirmation gate: the checker confirms the extracted and derived fields before any processing or staging; on by default for every Phase 1 configuration, switchable off per configuration by a wizard flag once extraction quality is proven; (2) commit approval: mandatory, never switchable; the checker sees the comparison table and approves or rejects on their own credentials, which commits the change. There is no human analyst role in the to-be; the automation is the maker. Segregation of duties is automation versus checker; the same checker may perform both points on one case.
- The business owner of the use case is the AI Business Owner under Article 2-1 of the AI Governance Standard: the same person, no separate
