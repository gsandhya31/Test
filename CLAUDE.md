...
- reference/   Read-only inputs:
  ...
  - Activity flow slides (original) and flows-area-1.md to flows-area-4.md (text conversions made in Session 0). Authoritative for as-is steps, input format, HITL and volumes where stated.
...
1. Area overview: activities, volumes, intake patterns, status table. Where input format, HITL or frequency is not stated in the flow material, include a "Business to populate" table (activity, input format, checker/HITL, frequency, comment) with blank cells, tagged [BUSINESS INPUT]. Never fill it from guesswork.
...
- intake: pattern (P1/P2/P3), mailbox or source, routing signal
...
- Source precedence: edm-activities-status.xlsx for scope and status; flow material for as-is steps, input format, HITL, volumes. Flag every conflict as [CONFLICT] rather than picking one silently.
...
- Three intake patterns exist and every activity must be tagged with one:
  - P1: email with a link to a webform / screen. Structured fields plus an unstructured comments field. Confirmed for 1.1 and 1.2.
