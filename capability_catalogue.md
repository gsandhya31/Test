...
5. To-be process, one sub-section per flow group that has an as-is (same grouping, regions and cross-references as section 2). Each step tagged with the platform skill and sub-mode it uses and marking the single human decision point (the checker). The automation is the maker. Every to-be is written in the same configuration shape (see Wizard-readiness). Where the as-is is pending business sign-off, the sub-section opens with "To-be drafted on an as-is pending business sign-off".
...
- decision points: the single human decision point, the checker, who approves or rejects on their own credentials; what the checker is shown (the comparison table); the rejection path
...
- Validate and Approve: (a) rules-based checks with no AI on the extracted and derived fields; (b) the single human decision point, the checker, who sees the comparison table (2.1 slide requirement, quoted in the catalogue: the approver receives a data comparison table contrasting requested against actual) and approves or rejects on their own credentials; (c) external approvals routed to Legal / FO / POC where the as-is requires them. Code-as-maker / human-as-checker is the control pattern inside this skill, not a separate utility.
...
- Route and Distribute: assignment of the staged change to a checker, escalation to Legal / FO / KYC, region routing.
- Sub-modes: every skill with more than one behaviour has named sub-modes in the catalogue (e.g. Validate and Approve (a) rules validation, (b) checker approval, (c) external approvals; Ingestion (a) mailbox, (b) EVE queue, (c) webform fetch, (d) identification). To-be steps tag at sub-mode level. A to-be may use one sub-mode of a skill and not the others; the uses / extends / new check runs per sub-mode.
