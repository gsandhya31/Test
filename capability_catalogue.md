...
5. To-be process, one sub-section per flow group that has an as-is (same grouping, regions and cross-references as section 2). Each step tagged with the platform skill and sub-mode it uses and marking the two checker decision points (extraction confirmation gate, commit approval). The automation is the maker; no human analyst step. Every to-be is written in the same configuration shape (see Wizard-readiness). Where the as-is is pending business sign-off, the sub-section opens with "To-be drafted on an as-is pending business sign-off".
