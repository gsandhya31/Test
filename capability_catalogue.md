...
- Standing rules: the to-be supplies a rejection path and a pending state in every configuration, stated as an addition to the as-is; writes execute on the analyst's own credentials, restated in every section 7. Neither is an open point.
- Draft v0.1 assumptions, listed in the front section and verified later: one mailbox (RDM-ORGS2) for all areas; writes via API where available, otherwise screen automation, per-system confirmation as a section 9 prerequisite owned by IT; IP4 intake mechanism as a section 9 prerequisite owned by IT/EVE.
